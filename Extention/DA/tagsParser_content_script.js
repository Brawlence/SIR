// TODO: parse name and author handle
function getImageTags() {
	...
	for (i = 0; i < tempArray.length; i++) {
		resultingTags.push( tempArray[i].innerText.substr(1) ); // cutting #
	};
	var eclipseTags = document.querySelectorAll("[href*='/tag/']");
	for (i = 0; i < eclipseTags.length; i++) {
		resultingTags.push( eclipseTags[i].innerText ); // no hash here #
	};
...
chrome.runtime.onMessage.addListener(
	function(request, sender, sendResponse) {
		if (request.order === "giffTags") {
			sendResponse({ tags: getImageTags(), origin: "DA" });
